...
export default function FAQSection() {
    ...
    return (
        <section className="flex flex-col md:flex-row w-full items-stretch justify-center py-24 gap-8 relative">
            ...
            {/* Right: Accordion */}
            <div className="flex-1 flex flex-col gap-4 h-fit w-full md:w-1/2 z-10">
                {faqs.map((faq, idx) => (
                    <div
                        key={idx}
                        className="rounded-xl shadow border border-gray-200 overflow-hidden mb-4 transition-all"
                    >
                        <button
                            className={`w-full text-left px-8 py-6 text-lg font-semibold flex items-center justify-between transition-colors duration-300 ${open === idx ? 'bg-[#2C4B7E] text-white' : 'bg-[rgba(255,255,255,0.10)] text-black'}`}
                            style={{
                        ...
                        >
                            {faq.question}
                            <span
                                className={`ml-4 text-2xl font-bold transform transition-transform duration-300 ${open === idx ? 'rotate-180' : ''}`}
                            >
                                {open === idx ? '−' : '+'}
                            </span>
                        </button>
                        <div
                            className={`px-8 pb-6 py-4 text-base bg-[rgba(255,255,255,0.10)] text-black overflow-hidden transition-all duration-500 ease-in-out ${open === idx ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'}`}
                            style={{
                                boxShadow: open === idx ? '0px 2px 4px 2px rgba(0, 0, 0, 0.10) inset' : 'none',
                            }}
                        >
